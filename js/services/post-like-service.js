...
export const removePostLike = async (postId) => {
  try {
    await httpClient.delete(API_CONFIG.ENDPOINTS.POST_LIKE(postId));
  } catch (error) {
...
export const togglePostLike = async (postId) => {
  try {
    const response = await httpClient.put(
      API_CONFIG.ENDPOINTS.POST_LIKE_TOGGLE(postId),
    );
...
/**
 * 게시글 좋아요 개수 및 사용자의 좋아요 여부 조회
 * @param {number} postId - 게시글 ID
 * @param {number} memberId - 회원 ID (선택)
 * @returns {Promise<Object>} { postId, likeCount, isLiked }
 */
export const getPostLikeCount = async (postId, memberId = null) => {
  try {
    const url = API_CONFIG.ENDPOINTS.POST_LIKE_COUNT(postId);

    // memberId가 있으면 헤더에 추가
    const response = await httpClient.get(url);

    return response;
  } catch (error) {
    console.error('게시글 좋아요 정보 조회 실패:', error);
    throw error;
  }
};
...
export const getLikesByPost = async (postId) => {
  try {
    ...
    console.error('게시글 좋아요 목록 조회 실패:', error);
    throw error;
  }
};
